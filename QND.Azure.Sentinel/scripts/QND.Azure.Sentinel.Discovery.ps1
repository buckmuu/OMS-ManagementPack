...
#
#
# SCRIPT	QND.Azure.Sentinel.Discovery.ps1
# AUTHOR	PROGEL SpA
...
#
# CHANGELOG
#	2019.12.05 1.0 [FGa] - First release.
#	2020.07.28 1.1 [FG] - Updated api-version
#
...
(
	...
	[Parameter(Mandatory = $false)] [System.String] $TraceLevel = 3
)
...
$SCRIPTNAME = 'QND.Azure.Sentinel.Discovery'
...
try
{
	...
	$Provider = Invoke-RestApi -Uri ([System.String]::Format('{0}/subscriptions/{1}/providers/Microsoft.SecurityInsights?api-version=2020-01-01', @($ResourceURI, $SubscriptionID))) -Authentication $Authentication
	
	if ($Provider -and @($Provider).Count -eq 1)
	{
		if ($Provider.registrationState -eq 'Registered')
		{
			$ClassInstance = $DiscoveryData.CreateClassInstance("$MPElement[Name='QND.Azure.Sentinel.Class']$")

			$ClassInstance.AddProperty("$MPElement[Name='System!System.Entity']/DisplayName$", ([System.String]::Format('Azure Sentinel ({0})', $SubscriptionID)))
			$ClassInstance.AddProperty("$MPElement[Name='QND.Azure.Sentinel.Class']/Id$", $Provider.id)
			$ClassInstance.AddProperty("$MPElement[Name='QND.Azure.Sentinel.Class']/Name$", ([System.String]::Format('Azure Sentinel ({0})', $SubscriptionID)))
			...
			$Message = [System.String]::Format('Azure Sentinel successfully discovered on subscription {0}.', $SubscriptionID)
			Write-Event -ID $EVENT_ID_SUCCESS -Type $EVENT_TYPE_INFORMATION -Message $Message -Severity $TRACE_VERBOSE
		}
	}
	else {throw [System.String]::Format("Unexpected result: Provider 'Microsoft.SecurityInsights' not found or found more than 1 on subscription '{0}'.", $SubscriptionID)}
	...
	Create-Event -ID $EVENT_ID_CONNECTIVITY -Type $EVENT_TYPE_INFORMATION -Message ([System.String]::Format('QND Azure Sentinel [{0}]', $SubscriptionID))
}
catch
{
